...
def update_board_incoming(pos_map, pos):
    pos = list(pos)
    w, h = 8, 8
    X = [['.' for x in range(w)] for y in range(h)] 
    Y = [['.' for x in range(w)] for y in range(h)] 
    
    for i in range(w):
        for j in range(h):
            X[i][j] = pos_map[10*(i+2)+1+j]
            
    for i in range(w):
        for j in range(h):
            Y[i][j] = pos[10*(i+2)+1+j]
            
    ## First Pass: Create Landscape

    Xo = []
    for i in range(8):
        for j in range(4):
            if X[j][i] == 'X':
                piece_to_add = random.choice(['r','n','b','q'])
                Y[j][i] = piece_to_add

      
    for i in range(w):
        for j in range(h):
            pos[10*(i+2)+1+j] = Y[i][j]
...
def initial_pos_gen():
    ...
    for k in range(i, i+j):
        piece_to_add = random.choice(['r','n','b','q'])
        X[Xo[k][1]][Xo[k][0]] = piece_to_add.upper()
        
    for i in range(w):
        for j in range(h):
            pos[10*(i+2)+1+j] = X[i][j]
    pos = "".join(pos)
    return pos
...
piece = { 'P': 100, 'N': 280, 'B': 320, 'R': 479, 'Q': 929, 'K': 60000, 'M': 0, 'H': 0}
pst = {
    'P': (   0,   0,   0,   0,   0,   0,   0,   0,
        ...
        for i, p in enumerate(self.board):
            ...
            for d in directions[p]:
                for j in count(i+d, d):
                    ...
                    if p in 'PNKH' or q.islower(): break
...
class Searcher:
    ...
    def search(self, pos, secs):
        ...
        try:
            ...
            print(self.tp_move.get(pos))
            print(self.tp_score)
            return self.tp_move.get(pos), 0
...
def main():
    pos = Position(initial_pos_gen(), 0, (True,True), (True,True), 0, 0)
    print_pos(pos)
    moves_list = []
    while True:
        searcher = Searcher()
        ...
        pos_drop = get_incoming(pos.board)
        print("Enemy Drops:")
        print_pos(Position(pos_drop, 0, (True,True), (True,True), 0, 0))
        ...
        move = None
        for _ in range(3):
            move, score = searcher.search(pos, secs=1.1+random.uniform(0,0.5))
            print("White move:", render(move[0]) + render(move[1]))
            moves_list.append(render(move[0]) + render(move[1]))
            pos = pos.move(move)
            print_pos(pos)
            if pos.score <= -MATE_LOWER:
                print("You won")
        ...
        for _ in range(3):
            ...
            moves_list.append(render(119-move[0]) + render(119-move[1]))
            pos = pos.move(move)
            print_pos(pos.rotate())
            if score == MATE_UPPER:
                print("Checkmate!")
        ...
        pos = Position(update_board_incoming(pos_drop, pos.board), pos.score, pos.wc, pos.bc, pos.ep, pos.kp)
        
        if len(moves_list)>7:
            if moves_list[-4:] == moves_list[-8:-4]:
                print('A tie!')
                break
